...
def summarize_pdf_with_qwen(chunks, filename):
    """
    Summarize a PDF using Qwen in batches so large PDFs do not
    ...
    if not final_answer.strip():
        raise RuntimeError(
            "Ollama returned an empty final summary."
        )
...
def summarize_pdf():
    ...
    try:
        ...
        summary = summarize_pdf_with_qwen(
            chunks,
            filename
        )
        ...
        return jsonify({
            "response": (
                "Qwen took too long to create the summary. "
                "Please try again."
            )
        }), 500
...
def _normalize_mcq_items(parsed, question_count):
    """Accept several reasonable Qwen JSON shapes and normalize them."""
...
def _extract_json_payload(content):
    """Extract the first usable JSON object/array from Qwen output."""
...
def generate_mcqs_with_qwen(chunks, filename, question_count=10):
...
"""
    ...
    if not content.strip():
        raise RuntimeError("Ollama returned an empty MCQ response.")

    parsed = _extract_json_payload(content)
    if parsed is None:
        raise RuntimeError(
            "Qwen returned text that could not be parsed as MCQ JSON."
        )
    ...
    if not valid_questions:
        raise RuntimeError(
            "Qwen returned JSON, but the MCQ fields were not in a usable format."
        )
...
def generate_mcq():
    ...
    try:
        ...
        questions = generate_mcqs_with_qwen(
            chunks,
            filename,
            requested_count
        )
        ...
        return jsonify({
            "response": (
                "Qwen took too long to generate the MCQs. "
                "Please try again."
            )
        }), 500
